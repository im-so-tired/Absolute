...
export default function useSort<T>(
	items: T[],
	initialSortBy: { path: keyof T | 'bookings'; order: Order }
) {
	...
	const sortedItems = stableSort(
		items,
		getComparator(sortBy.order, sortBy.path as any)
	)

	return { sortedItems, sortBy, setSortBy, handleRequestSort }
}
// .map(room => {...room, bookings: bookings?.filter(item => item.roomId === room._id)})
